...
| --- | --- | --- |
| `CREATE [OR REPLACE] TABLE x ... AS query` | `type: "table"` | `PARTITION BY` / `CLUSTER BY` / `OPTIONS` mapped to config |
| `CREATE TABLE IF NOT EXISTS x AS query` | `operations` (default) | `--if-not-exists table` opts into a typed action and reports the lost guard |
...
| `CREATE TABLE ... LIKE/CLONE/COPY` | `operations` | (`FALLBACK_OPERATIONS`) |
| `CREATE TABLE x (schema)` (no `AS`) | `operations` (default) or `declaration` | `--plain-create declaration` drops the DDL and declares the source |
| `CREATE EXTERNAL TABLE` / `SNAPSHOT` | `operations` | Tracked as a writer of the target |
